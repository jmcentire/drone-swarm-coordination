...
Zenodo under DOI
\href{https://doi.org/10.5281/zenodo.19954678}{10.5281/zenodo.19954678}
...
Across all three mission classes, the architecture is treated as a
\emph{composition} of three load-bearing components: broadcast as shared
state (information distribution), deterministic per-drone compute (local
reasoning), and majority-vote tie-break under degraded comms (decision
aggregation). All three are required; removing any one breaks the
architecture. The empirical work below establishes each component's
load-bearing role.
...
cost field rather than being told it. The architecture's contribution is
algorithm-agnostic: a state-of-the-art partially-known-terrain planner
(D* Lite, frontier-based exploration, value-of-information-driven
manifold selection) would close the gap further; the architecture itself
is unchanged.
...
\item
  \textbf{Bandyopadhyay-Chung consensus filter is dominated across T ∈
  \{1, 5, 20, 50\}.} B-C performance is essentially flat across
  consensus-round count: T=1 produces 12/20 finds on lost\_at\_sea at
  62.6 mean iters; T=50 produces 12/20 at 61.2 mean iters. The
  broadcast-substrate's per-tick decision aggregation dominates B-C's
  T-round consensus loop in the operational SAR regime, regardless of T.
  The dominance is structural to the engineering composition, not an
  artifact of T-tuning.
\item
...
\item
  \textbf{The asymmetric-deaf cliff confirms the n/2 majority-vote
  threshold:} 10\% deaf, 20/20; 25\% deaf, 18-20/20; 50\% deaf, 0/20;
  100\% deaf, 0/20. We \emph{confirm empirically} that the
  ...
  predicted by voting theory in the SAR setting.
\item
...
work). The SAROPS centralized comparator is grounded in Kratzke, Stone,
Frost (2010); Stone (1975, 2007); Stone-Royset-Washburn (2016); Allen \&
Plourde (1999); Allen (2005); Breivik \& Allen (2008), with all 85
object-class Leeway parameters extracted from OpenDrift's
\texttt{OBJECTPROP.DAT} (the Art Allen → Breivik transcription of the
...
requires larger N. The bench is a simulation; real-platform sensor and
comms calibration require deployment data we do not have.
...
\subsubsection{Lemma 9.5 (ETA-deadline quiescence under packet
loss)}\label{lemma-9.5-eta-deadline-quiescence-under-packet-loss}

Suppose every en-route drone d ∈ D broadcasts
\texttt{STATE\ =\ EN\_ROUTE} with current ETA estimate ETA\_d at
intervals τ\_d \textgreater{} 0, and arrived drones broadcast no
transit-state messages. Let ETA\_max = max\_d ETA\_d over the en-route
set, δ \textgreater{} 0 a tolerance margin, and T\_max = ETA\_max + δ.
Assume an independent per-message delivery failure probability p
\textless{} 1 between any sender-receiver pair. Then for every drone d'
that survives to T\_max:

\begin{enumerate}
\def\labelenumi{(\alph{enumi})}
\item
  the probability that d' fails to receive \emph{any} report containing
  ETA\_max during the interval {[}t\_0, ETA\_max{]} decays as p\^{}k
  where k is the number of reports broadcast by the slowest drone in
  that interval; and
\item
  at time T\_max, every surviving d' that has not received any EN\_ROUTE
  broadcast in the window (T\_max − τ\emph{, T\_max{]}, where τ} is the
  minimum broadcast interval over the en-route set, deterministically
  transitions to the next phase.
\end{enumerate}

\textbf{Proof.} (a) The slowest drone broadcasts ⌈(ETA\_max − t\_0)/τ⌉
times in the interval. Independent loss with probability p makes total
failure probability ≤ p\^{}k, which is exponentially small for modest k
regardless of any single recipient's per-message loss rate. The
aggregation over the entire en-route set (each drone broadcasts its own
ETA estimate, recipients keep the maximum observed) only strengthens
this, since recipients can also infer ETA\_max from any drone whose own
ETA estimate equals or exceeds it.
...
\tightlist
\item
  The transition trigger is \emph{negative evidence}: absence of
  EN\_ROUTE traffic in the deadline window. A perfectly silent window on
  the broadcast channel is observationally identical to ``all drones
...
\textbf{Remark (channel-denial residual).} Lemma 9.5's transition
trigger cannot distinguish ``everyone arrived'' from ``the channel
itself went silent for non-arrival reasons (jamming, fade).'' This is a
property of the substrate, not the protocol. In any operational
deployment the broadcast carries non-transition traffic (position
telemetry, sensor data, command/control); a drone observing total
channel silence --- \emph{no} broadcasts of any kind for the prior N
seconds --- can defer the phase transition pending channel recovery.
This adds no protocol messages and is a sanity check on the substrate
itself.

\subsubsection{Theorem 2.5 (Mid-flight reconfiguration
consensus)}\label{theorem-2.5-mid-flight-reconfiguration-consensus}

Let M, M' be two manifolds with M' arriving at the swarm before the
transit toward M has completed. Let \texttt{prior\_end\_state(D,\ M)}
denote the leaf coordinates each drone would occupy under ASSIGN(D,
T(M)) upon completion. If every drone d ∈ D runs ASSIGN(D, T(M')) using
input positions either (i) prior\_end\_state(D, M) or (ii) a
broadcast-snapshot of live positions latched at a common logical tick
t*, then all drones derive identical assignments to M'.

\textbf{Proof.} Both inputs are byte-identical across drones. For (i),
prior\_end\_state(D, M) is the output of ASSIGN(D, T(M)), which is
deterministic across drones by Theorem 1; therefore every drone computes
the same leaf-coordinate input. For (ii), Lemma 9 gives broadcast
consistency at any read on a quiescent window; for a non-quiescent
window, common consensus on t* is required (the same machinery as
Theorem 2 supplies, applied to a snapshot tick rather than an all-locked
tick). In both cases ASSIGN's determinism (Theorem 1) maps the
byte-identical input to a byte-identical output; the swarm reaches
consensus on the M' assignment without any global drone-to-leaf mapping
ever being computed by any single drone. ∎

\textbf{Remark (option choice).} Option (i) is preferred in deployment
...
already computed locally. Option (ii) is more path-efficient (drones
recompute against where they actually are, not where they would have
ended up) but pays the snapshot-coordination cost.
